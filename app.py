...
<div class="body">
  ...
  <div class="main">
    ...
    <div class="panel active" id="panel-generate">
      <div class="gen-card">
        ...
        <div class="approval-panel" id="template-panel">
          ...
          <p>Pick one or more formats to build. This video will use the default style you saved in the 🎨 Templates tab.</p>

          <div class="tpl-grid" id="template-grid"></div>

          <details class="tpl-style-panel" id="template-style-details">
            <summary style="cursor:pointer;font-weight:600;padding:8px 0;">
              ⚙️ Override style for this video only (optional)
            </summary>
            <p style="font-size:12px;color:var(--sub);margin:6px 0 12px;">
              Changes here apply <strong>only to this video</strong> — your saved default in the Templates tab won't change.
            </p>
    ...
    <div class="panel" id="panel-templates">
      ...
      <p style="color:var(--sub);">Click any element in the preview to edit its properties. Changes update live.</p>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;height:calc(100vh - 200px);">
        <!-- LEFT: Visual Preview -->
        <div style="background:#0d0d0d;border-radius:10px;overflow:hidden;display:flex;flex-direction:column;">
          <div style="flex:1;overflow:auto;display:flex;align-items:center;justify-content:center;padding:10px;">
            <svg id="editor-preview" viewBox="0 0 1280 720" style="max-width:100%;max-height:100%;cursor:pointer;filter:drop-shadow(0 4px 12px rgba(0,0,0,0.5));">
            </svg>
          </div>
        </div>

        <!-- RIGHT: Property Inspector -->
        <div style="background:#141414;border-radius:10px;overflow-y:auto;padding:16px;border:1px solid #2a2a2a;">
          <h4 id="editor-selected-label" style="margin:0 0 16px 0;color:#fff;">Select an element</h4>
          <div id="editor-props-panel"></div>
          <div style="margin-top:24px;display:flex;gap:8px;">
            <button class="btn-approve-templates" onclick="editorSaveDefaults()" style="flex:1;">💾 Save as Default</button>
            <button class="btn-add-sentence" onclick="editorResetLayout()" style="flex:1;">↺ Reset</button>
          </div>
        </div>
      </div>
    </div>
...
function showPanel(name) {
  ...
  if (name === 'templates') initEditorPanel();
}
...
// ── Single source of truth for template previews ──────────────
// Both the standalone Templates-tab editor AND the in-flow Template
// Studio (after image approval) call this SAME function, so they can
// never visually drift apart from each other or from the real renderer.
function buildAccurateTemplateSVG(style, opts) {
  opts = opts || {};
  const defaults = {
    body_bg: '#000000', banner_color: '#FFD100', bottom_color: '#121212', frame_color: '#C81414',
    logo_position: 'top-right', logo_shape: 'circle', corner_radius: 28, container_radius: 36,
    headline_size: 64, caption_size: 30,
    top_height: 108, top_fixed_text: 'आज की 50 बड़ी खबरें', top_text: 'मुख्य समाचार आम आदमी के काम की खबरें',
    top_text_size: 34, top_text_color: '#000000',
    logo_size: 84, logo_bg: '#FFFFFF', logo_border: '#333333',
    mid_container_margin_top: 0, mid_container_margin_right: 22, mid_container_margin_bottom: 0, mid_container_margin_left: 22,
    mid_container_padding_top: 0, mid_container_padding_right: 0, mid_container_padding_bottom: 0, mid_container_padding_left: 0,
    image_border: 6,
    breaking_text: 'BREAKING NEWS', breaking_text_color: '#FFFFFF', breaking_bg: '#8E0E0E', breaking_outline: '#FFFFFF',
    breaking_box_width: 0, breaking_box_height: 0, breaking_text_align: 'center',
    caption_radius: 12,
    caption_padding_top: 20, caption_padding_right: 20, caption_padding_bottom: 20, caption_padding_left: 20,
    caption_margin_top: 18, caption_margin_right: 0, caption_margin_bottom: 0, caption_margin_left: 0,
    bottom_height: 72, bottom_text: 'अपने दोस्तों के साथ फेसबुक और व्हाट्सएप पर शेयर जरूर करें',
    bottom_text_size: 28, bottom_text_color: '#FFFFFF', bottom_logo_size: 50,
  };
  // Only override a default when the incoming value is actually defined —
  // an explicit `undefined` key (e.g. from a stale/partial style object)
  // must NOT blank out a sane default, or every numeric field silently
  // becomes NaN in the SVG.
  const s = Object.assign({}, defaults);
  if (style) {
    for (const k in style) {
      if (style[k] !== undefined && style[k] !== null && style[k] !== '') s[k] = style[k];
    }
  }

  const W = 1280, H = 720;
  const MID_H = H - s.top_height - s.bottom_height;
  const midY = s.top_height;
  const click = (key) => opts.interactive ? ` onclick="editorSelect('${key}')" style="cursor:pointer;"` : '';

  // Mirrors landscape_16_9.py's geometry so the preview never drifts from
  // the real render.
  const autoPad = Math.max(Math.round(MID_H * 0.045), Math.round(s.container_radius * 0.7));
  const pTop = s.mid_container_padding_top || autoPad;
  const pRight = s.mid_container_padding_right || autoPad;
  const pBottom = s.mid_container_padding_bottom || autoPad;
  const pLeft = s.mid_container_padding_left || autoPad;

  const containerX = s.mid_container_margin_left;
  const containerY = midY + s.mid_container_margin_top;
  const containerW = W - s.mid_container_margin_left - s.mid_container_margin_right;
  const containerH = Math.max(80, MID_H - s.mid_container_margin_top - s.mid_container_margin_bottom);
  const CONTENT_H = Math.max(60, containerH - pTop - pBottom);
  const GAP = 12;
  const HALF_W = Math.max(80, Math.floor((containerW - pLeft - pRight - GAP) / 2));
  const contentY = containerY + pTop;
  const leftX = containerX + pLeft;
  const rightX = leftX + HALF_W + GAP;

  const pillH = s.breaking_box_height ? s.breaking_box_height : s.headline_size + 20;
  const pillW = s.breaking_box_width ? Math.min(s.breaking_box_width, HALF_W) : HALF_W;
  const textAnchor = s.breaking_text_align === 'left' ? 'start' : s.breaking_text_align === 'right' ? 'end' : 'middle';
  const textX = s.breaking_text_align === 'left' ? rightX + 30 : s.breaking_text_align === 'right' ? rightX + pillW - 30 : rightX + pillW / 2;

  const capX = rightX + s.caption_margin_left;
  const capY = contentY + pillH + s.caption_margin_top;
  const capW = Math.max(60, HALF_W - s.caption_margin_left - s.caption_margin_right);
  const capH = Math.max(60, CONTENT_H - pillH - s.caption_margin_top - s.caption_margin_bottom);

  return `
    <rect x="0" y="0" width="${W}" height="${H}" fill="${s.body_bg}"${click('canvas')}/>

    <rect x="0" y="0" width="${W}" height="${s.top_height}" fill="${s.banner_color}"${click('topBanner')}/>
    <text x="20" y="${s.top_height/2+6}" font-size="${s.top_text_size}" fill="${s.top_text_color}" font-weight="bold"${click('topBanner')}>${s.top_fixed_text}</text>
    <text x="${380 + (s.top_fixed_text ? s.top_fixed_text.length * s.top_text_size * 0.42 : 0)}" y="${s.top_height/2+6}" font-size="${s.top_text_size}" fill="${s.top_text_color}" opacity="0.75"${click('topBanner')}>${s.top_text} →</text>

    <g${click('logo')}>
      ${s.logo_shape === 'circle'
        ? `<circle cx="${s.logo_position === 'top-right' ? W - s.logo_size/2 - 20 : s.logo_size/2 + 20}" cy="${s.top_height/2}" r="${s.logo_size/2}" fill="${s.logo_bg}" stroke="${s.logo_border}" stroke-width="2"/>`
        : `<rect x="${s.logo_position === 'top-right' ? W - s.logo_size - 20 : 20}" y="${s.top_height/2 - s.logo_size/2}" width="${s.logo_size}" height="${s.logo_size}" rx="4" fill="${s.logo_bg}" stroke="${s.logo_border}" stroke-width="2"/>`
      }
    </g>

    <rect x="${containerX}" y="${containerY}" width="${containerW}" height="${containerH}" rx="${s.container_radius}" fill="${s.frame_color}"${click('midContainer')}/>

    <g${click('imageArea')}>
      <rect x="${leftX}" y="${contentY}" width="${HALF_W}" height="${CONTENT_H}" rx="${s.corner_radius}" fill="#FFFFFF"/>
      <rect x="${leftX + s.image_border}" y="${contentY + s.image_border}" width="${HALF_W - s.image_border*2}" height="${CONTENT_H - s.image_border*2}" rx="${Math.max(0, s.corner_radius - s.image_border)}" fill="#555555"/>
      <text x="${leftX + HALF_W/2}" y="${contentY + CONTENT_H/2}" font-size="16" fill="#ddd" text-anchor="middle" pointer-events="none">IMAGE/VIDEO</text>
    </g>

    <g${click('breakingBox')}>
      <rect x="${rightX}" y="${contentY}" width="${pillW}" height="${pillH}" rx="${pillH/2}" fill="${s.breaking_bg}" stroke="${s.breaking_outline}" stroke-width="3"/>
      <text x="${textX}" y="${contentY + pillH/2 + 8}" font-size="${s.headline_size*0.7}" fill="${s.breaking_text_color}" text-anchor="${textAnchor}" font-weight="bold" pointer-events="none">${s.breaking_text}</text>
    </g>

    <rect x="${capX}" y="${capY}" width="${capW}" height="${capH}" rx="${s.caption_radius}" fill="${s.banner_color}"${click('captionBox')}/>
    <text x="${capX + s.caption_padding_left}" y="${capY + capH/2 + 6}" font-size="${s.caption_size*0.8}" fill="#000000" pointer-events="none">Caption text here</text>

    <rect x="0" y="${H - s.bottom_height}" width="${W}" height="${s.bottom_height}" fill="${s.bottom_color}"${click('bottomBanner')}/>
    <circle cx="${14 + s.bottom_logo_size/2}" cy="${H - s.bottom_height/2}" r="${s.bottom_logo_size/2}" fill="#FFFFFF" stroke="#333333" stroke-width="2"${click('bottomBanner')}/>
    <text x="${14 + s.bottom_logo_size + 20}" y="${H - s.bottom_height/2 + 8}" font-size="${s.bottom_text_size*0.9}" fill="${s.bottom_text_color}"${click('bottomBanner')}>${s.bottom_text}</text>
  `;
}

// Converts the editor's internal _editorLayout shape into the flat
// style-dict shape used everywhere else (server, saved defaults,
// buildAccurateTemplateSVG). Single source of truth for that mapping.
function layoutToStyle(layout) {
  return {
    banner_color: layout.topBanner.bg,
    bottom_color: layout.bottomBanner.bg,
    frame_color: layout.midContainer.bg,
    body_bg: layout.canvas.bg,
    logo_position: layout.logo.pos === 'top-left' ? 'top-left' : 'top-right',
    logo_shape: layout.logo.shape,
    container_radius: layout.midContainer.radius,
    corner_radius: layout.imageArea.radius,
    headline_size: layout.breakingBox.textSize,
    caption_size: layout.captionBox.textSize,
    top_height: layout.topBanner.h,
    top_padding: layout.topBanner.mt,
    top_margin_lr: layout.topBanner.ml,
    top_fixed_text: layout.topBanner.fixedText,
    top_text: layout.topBanner.text,
    top_text_size: layout.topBanner.textSize,
    top_text_color: layout.topBanner.textColor,
    logo_size: layout.logo.size,
    logo_bg: layout.logo.bg,
    logo_border: layout.logo.border,
    logo_path: layout.logo.customPath || '',
    mid_border_width: layout.midContainer.border || 0,
    mid_container_margin_top: layout.midContainer.marginTop,
    mid_container_margin_right: layout.midContainer.marginRight,
    mid_container_margin_bottom: layout.midContainer.marginBottom,
    mid_container_margin_left: layout.midContainer.marginLeft,
    mid_container_padding_top: layout.midContainer.paddingTop,
    mid_container_padding_right: layout.midContainer.paddingRight,
    mid_container_padding_bottom: layout.midContainer.paddingBottom,
    mid_container_padding_left: layout.midContainer.paddingLeft,
    image_border: layout.imageArea.border,
    image_bg: layout.imageArea.bg,
    image_gradient_alpha: layout.imageArea.gradientAlpha,
    breaking_text: layout.breakingBox.text,
    breaking_text_color: layout.breakingBox.textColor,
    breaking_bg: layout.breakingBox.bg,
    breaking_outline: layout.breakingBox.outline,
    breaking_box_width: layout.breakingBox.width,
    breaking_box_height: layout.breakingBox.height,
    breaking_font: layout.breakingBox.font,
    breaking_text_align: layout.breakingBox.textAlign,
    caption_radius: layout.captionBox.radius,
    caption_align: layout.captionBox.align,
    caption_padding_top: layout.captionBox.paddingTop,
    caption_padding_right: layout.captionBox.paddingRight,
    caption_padding_bottom: layout.captionBox.paddingBottom,
    caption_padding_left: layout.captionBox.paddingLeft,
    caption_margin_top: layout.captionBox.marginTop,
    caption_margin_right: layout.captionBox.marginRight,
    caption_margin_bottom: layout.captionBox.marginBottom,
    caption_margin_left: layout.captionBox.marginLeft,
    bottom_height: layout.bottomBanner.h,
    bottom_padding: layout.bottomBanner.mt,
    bottom_margin_lr: layout.bottomBanner.ml,
    bottom_text: layout.bottomBanner.text,
    bottom_text_size: layout.bottomBanner.textSize,
    bottom_text_color: layout.bottomBanner.textColor,
    bottom_logo_size: layout.bottomBanner.logoSize,
  };
}
...
function renderAllPreviews() {
  _templateList.forEach(t => {
    ...
    if (!box) return;
    if (t.key === 'landscape_16_9') {
      box.innerHTML = `<svg viewBox="0 0 1280 720" style="width:100%;height:100%;">${buildAccurateTemplateSVG(_currentStyle, { interactive: false })}</svg>`;
    } else {
      box.innerHTML = buildTemplateSVG(t, _currentStyle);
    }
  });
}
...
// ── Visual Layout Editor (full control mode) ──────────────────
const LAYOUT_STORAGE_KEY = 'realNewsLayoutEditor';
const LAYOUT_DEFAULTS = {
  canvas: { bg: '#000000' },
  topBanner: { h: 108, mt: 0, ml: 0, mr: 0, bg: '#FFD100', fixedText: 'आज की 50 बड़ी खबरें', text: 'मुख्य समाचार आम आदमी के काम की खबरें', textSize: 34, textColor: '#000000' },
  logo: { size: 84, pos: 'top-right', shape: 'circle', bg: '#FFFFFF', border: '#333333', customPath: '', previewUrl: '' },
  midContainer: { bg: '#C81414', radius: 36, border: 0, marginTop: 0, marginRight: 22, marginBottom: 0, marginLeft: 22, paddingTop: 0, paddingRight: 0, paddingBottom: 0, paddingLeft: 0 },
  imageArea: { radius: 28, border: 6, bg: '#FFFFFF', gradientAlpha: 0.45 },
  breakingBox: { bg: '#8E0E0E', outline: '#FFFFFF', textSize: 64, textColor: '#FFFFFF', text: 'BREAKING NEWS', width: 0, height: 0, font: 'impact', textAlign: 'center' },
  captionBox: { bg: '#FFD100', textSize: 30, textColor: '#000000', radius: 12, align: 'center', paddingTop: 20, paddingRight: 20, paddingBottom: 20, paddingLeft: 20, marginTop: 18, marginRight: 0, marginBottom: 0, marginLeft: 0 },
...
let _editorLayout = { ...LAYOUT_DEFAULTS };
let _editorSelected = null;

function deepMergeLayout(defaults, saved) {
  // Merges each nested element object individually (topBanner, logo, etc.)
  // instead of a shallow top-level spread — a shallow spread lets an OLD
  // saved sub-object (from before a new field like `outline` or `canvas`
  // existed) silently wipe that new field out with `undefined`, which is
  // exactly what caused the NaN SVG errors and "undefined" text.
  const out = {};
  for (const key in defaults) {
    const base = defaults[key];
    const savedPart = saved && saved[key];
    out[key] = (base && typeof base === 'object' && !Array.isArray(base))
      ? Object.assign({}, base, savedPart && typeof savedPart === 'object' ? savedPart : {})
      : (savedPart !== undefined ? savedPart : base);
  }
  return out;
}
...
function loadEditorLayout() {
  try {
    ...
    _editorLayout = deepMergeLayout(LAYOUT_DEFAULTS, saved);
  } catch (e) {
    _editorLayout = JSON.parse(JSON.stringify(LAYOUT_DEFAULTS));
  }
}

function saveEditorLayout() {
  try {
    localStorage.setItem(LAYOUT_STORAGE_KEY, JSON.stringify(_editorLayout));
  } catch (e) {}
}

function editorSaveDefaults() {
  saveEditorLayout();
  const synthStyle = layoutToStyle(_editorLayout);
  saveStyle(synthStyle, ['landscape_16_9']);
  alert('✓ Template saved — all properties will apply to your next video');
}
...
function renderEditorPreview() {
  const svg = document.getElementById('editor-preview');
  svg.innerHTML = buildAccurateTemplateSVG(layoutToStyle(_editorLayout), { interactive: true });
}

function editorSelect(key) {
  _editorSelected = key;
  renderEditorProps();
}
...
function renderEditorProps() {
  ...
  const label = document.getElementById('editor-selected-label');
  
  if (!_editorSelected) {
    label.textContent = 'Select an element';
    panel.innerHTML = '<p style="color:var(--sub);font-size:12px;margin:0;">Click any part of the preview to edit it.</p>';
    return;
  }
  
  const sel = _editorSelected;
  const elem = _editorLayout[sel];
  label.textContent = sel.replace(/([A-Z])/g, ' $1').trim();
  
  let html = '';
  const inputHandler = (key, prop) => `onchange="editorUpdateProp('${key}','${prop}',this.value);renderEditorPreview();"`;
  const rangeHandler = (key, prop) => `oninput="editorUpdateProp('${key}','${prop}',this.value);renderEditorPreview();"`;
  
  // Generic property editor based on element type
  for (const [prop, val] of Object.entries(elem)) {
    const id = `ed-${sel}-${prop}`;
    html += '<label class="tpl-ctrl" style="margin-bottom:12px;"><span style="display:block;font-size:11px;color:var(--sub);margin-bottom:4px;">';
    html += prop.replace(/([A-Z])/g, ' $1').toLowerCase();
    html += '</span>';
    
    const isHexColor = typeof val === 'string' && /^#[0-9A-Fa-f]{6}$/.test(val);
    if (isHexColor) {
      html += `<input type="color" id="${id}" value="${val}" ${inputHandler(sel, prop)} style="width:100%;height:32px;border-radius:6px;border:none;cursor:pointer;"/>`;
    } else if (prop === 'pos' || prop === 'shape' || prop === 'align' || prop === 'textAlign' || prop === 'font') {
      html += `<select id="${id}" ${inputHandler(sel, prop)} style="width:100%;padding:6px;border-radius:6px;border:1px solid #333;background:#0d0d0d;color:#fff;">`;
      if (prop === 'pos') {
        html += `<option value="top-left" ${val==='top-left' ? 'selected' : ''}>Top Left</option><option value="top-right" ${val==='top-right' ? 'selected' : ''}>Top Right</option>`;
      } else if (prop === 'shape') {
        html += `<option value="circle" ${val==='circle' ? 'selected' : ''}>Circle</option><option value="square" ${val==='square' ? 'selected' : ''}>Square</option>`;
      } else if (prop === 'align' || prop === 'textAlign') {
        html += `<option value="left" ${val==='left' ? 'selected' : ''}>Left</option><option value="center" ${val==='center' ? 'selected' : ''}>Center</option><option value="right" ${val==='right' ? 'selected' : ''}>Right</option>`;
      } else if (prop === 'font') {
        html += `<option value="impact" ${val==='impact' ? 'selected' : ''}>Impact (bold display)</option><option value="devanagari" ${val==='devanagari' ? 'selected' : ''}>Devanagari Bold</option><option value="system" ${val==='system' ? 'selected' : ''}>System default</option>`;
      }
      html += '</select>';
    } else if (prop.includes('text') || prop === 'text') {
      html += `<input type="text" id="${id}" value="${val}" ${inputHandler(sel, prop)} style="width:100%;padding:6px;border-radius:6px;border:1px solid #333;background:#0d0d0d;color:#fff;"/>`;
    } else if (typeof val === 'number') {
      html += `<input type="number" id="${id}" value="${val}" ${rangeHandler(sel, prop)} style="width:100%;padding:6px;border-radius:6px;border:1px solid #333;background:#0d0d0d;color:#fff;"/>`;
    } else {
      html += `<input type="text" id="${id}" value="${val}" ${inputHandler(sel, prop)} style="width:100%;padding:6px;border-radius:6px;border:1px solid #333;background:#0d0d0d;color:#fff;"/>`;
    }
    html += '</label>';
  ...
  // Logo upload — applies to both top and bottom logo (they share one image)
  if (sel === 'logo') {
    const uploadBlock = document.createElement('div');
    uploadBlock.style.cssText = 'margin-top:16px;padding-top:16px;border-top:1px solid #2a2a2a;';
    uploadBlock.innerHTML = `
      <span style="display:block;font-size:11px;color:var(--sub);margin-bottom:6px;">Upload custom logo (used for both top &amp; bottom banners)</span>
      <input type="file" id="logo-upload-input" accept="image/png,image/jpeg,image/webp" onchange="handleLogoUpload(this)" style="width:100%;font-size:12px;color:#fff;margin-bottom:8px;"/>
      ${_editorLayout.logo.previewUrl ? `
        <div style="display:flex;align-items:center;gap:10px;">
          <img src="${_editorLayout.logo.previewUrl}" style="width:40px;height:40px;object-fit:contain;background:#fff;border-radius:6px;padding:4px;"/>
          <button onclick="removeCustomLogo()" style="font-size:12px;padding:6px 10px;border-radius:6px;border:1px solid #333;background:#1a1a1a;color:#fff;cursor:pointer;">Remove, use default</button>
        </div>` : '<span style="font-size:11px;color:var(--sub);">Using bundled default logo</span>'}
      <p id="logo-upload-status" style="font-size:11px;color:var(--sub);margin-top:6px;"></p>
    `;
    panel.appendChild(uploadBlock);
  }
}
...
function handleLogoUpload(inputEl) {
  ...
  fetch('/upload_logo', { method: 'POST', body: form })
    .then(r => r.json())
    .then(d => {
      if (!d.ok) { if (status) status.textContent = 'Error: ' + (d.error || 'upload failed'); return; }
      _editorLayout.logo.customPath = d.logo_path;
      _editorLayout.logo.previewUrl = d.preview_url;
      saveEditorLayout();
      renderEditorPreview();
      renderEditorProps();
    })
...
function editorUpdateProp(elemKey, prop, val) {
  if (typeof _editorLayout[elemKey][prop] === 'number') {
    _editorLayout[elemKey][prop] = parseInt(val) || 0;
  } else {
    _editorLayout[elemKey][prop] = val;
  }
  saveEditorLayout();
}
...
def approve_templates(job_id):
    ...
    for key, default_val in DEFAULT_STYLE.items():
        ...
        if key == 'logo_shape' and val not in ('circle', 'square'):
            continue
        if key == 'caption_align' and val not in ('left', 'center', 'right'):
            continue
        if key == 'breaking_text_align' and val not in ('left', 'center', 'right'):
            continue
        if key == 'breaking_font' and val not in ('impact', 'devanagari', 'system'):
            continue
        
        # Integer validations (sizes, heights, paddings)
        if key in ('corner_radius', 'container_radius', 'headline_size', 'caption_size',
                   'top_height', 'top_padding', 'top_margin_lr', 'top_text_size',
                   'logo_size', 'mid_border_width', 'image_border', 'bottom_height',
                   'bottom_padding', 'bottom_margin_lr', 'bottom_text_size', 'caption_radius',
                   'bottom_logo_size',
                   'mid_container_margin_top', 'mid_container_margin_right',
                   'mid_container_margin_bottom', 'mid_container_margin_left',
                   'mid_container_padding_top', 'mid_container_padding_right',
                   'mid_container_padding_bottom', 'mid_container_padding_left',
                   'breaking_box_width', 'breaking_box_height',
                   'caption_padding_top', 'caption_padding_right',
                   'caption_padding_bottom', 'caption_padding_left',
                   'caption_margin_top', 'caption_margin_right',
                   'caption_margin_bottom', 'caption_margin_left'):
            try:
                val = int(val)
            except (TypeError, ValueError):
                continue
        
        # Float validations (gradient alpha)
        if key == 'image_gradient_alpha':
            try:
                val = float(val)
                if not (0 <= val <= 1):
                    continue
        ...
        # Color hex validations
        if key in ('banner_color', 'bottom_color', 'frame_color', 'body_bg', 'top_text_color',
                   'logo_bg', 'logo_border', 'image_bg', 'breaking_text_color', 'breaking_bg',
                   'breaking_outline', 'bottom_text_color'):
            if not isinstance(val, str) or not val.startswith('#') or len(val) != 7:
                continue
            try:
                int(val[1:], 16)  # Validate hex
            except ValueError:
                continue
        ...
        # Logo path: only accept empty (= use bundled default) or a path
        # we generated ourselves via /upload_logo — never trust an
        # arbitrary client-supplied filesystem path.
        if key == 'logo_path':
            if not isinstance(val, str):
                continue
            if val:
                custom_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'custom_logos')
                real = os.path.realpath(val)
                if not real.startswith(os.path.realpath(custom_dir) + os.sep) or not os.path.exists(real):
                    continue
...
def upload_logo():
    """Accepts a logo image upload, saves it, and returns a URL the
    frontend can preview and a server-side path the renderer can use."""
    ...
    if ext not in ('.png', '.jpg', '.jpeg', '.webp'):
        return jsonify(ok=False, error='Use PNG, JPG, or WEBP'), 400

    custom_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'custom_logos')
    os.makedirs(custom_dir, exist_ok=True)
    fname = f'logo_{uuid.uuid4().hex[:10]}{ext}'
    save_path = os.path.join(custom_dir, fname)
    f.save(save_path)

    return jsonify(ok=True, logo_path=save_path, preview_url=f'/custom_logo/{fname}')


@app.route('/custom_logo/<path:fname>')
def serve_custom_logo(fname):
    custom_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets', 'custom_logos')
    return send_file(os.path.join(custom_dir, fname))
