...
function buildAccurateTemplateSVG(style, opts) {
  ...
  const defaults = {
    ...
    mid_container_margin: 22, mid_container_padding: 0,
    image_border: 6,
    ...
    caption_radius: 12, caption_padding: 20, caption_margin: 18,
  ...
  // the real render.
  const MARGIN_X = s.mid_container_margin;
  const PAD = s.mid_container_padding ? s.mid_container_padding : Math.max(Math.round(MID_H * 0.045), Math.round(s.container_radius * 0.7));
  const CONTENT_H = MID_H - 2 * PAD;
  const CONTAINER_W = W - 2 * MARGIN_X;
  const GAP = 12;
  const HALF_W = Math.floor((CONTAINER_W - 3 * PAD - GAP) / 2);
  const contentY = midY + PAD;
  const leftX = MARGIN_X + PAD;
  ...
  const captionY = contentY + pillH + s.caption_margin;
  const captionH = Math.max(60, CONTENT_H - pillH - s.caption_margin);
  ...
  return `
    ...
    <rect x="${MARGIN_X}" y="${midY}" width="${CONTAINER_W}" height="${MID_H}" rx="${s.container_radius}" fill="${s.frame_color}"${click('midContainer')}/>
    ...
    <rect x="${rightX}" y="${captionY}" width="${HALF_W}" height="${captionH}" rx="${s.caption_radius}" fill="${s.banner_color}"${click('captionBox')}/>
    <text x="${rightX + s.caption_padding}" y="${captionY + captionH/2 + 6}" font-size="${s.caption_size*0.8}" fill="#000000" pointer-events="none">Caption text here</text>
...
function layoutToStyle(layout) {
  return {
    ...
    mid_container_margin: layout.midContainer.margin,
    mid_container_padding: layout.midContainer.padding,
    ...
    caption_padding: layout.captionBox.padding,
    caption_margin: layout.captionBox.margin,
...
const LAYOUT_DEFAULTS = {
  ...
  midContainer: { bg: '#C81414', radius: 36, border: 0, margin: 22, padding: 0 },
  ...
  captionBox: { bg: '#FFD100', textSize: 30, textColor: '#000000', radius: 12, align: 'center', padding: 20, margin: 18 },
...
def approve_templates(job_id):
    ...
    for key, default_val in DEFAULT_STYLE.items():
        ...
        if key in ('corner_radius', 'container_radius', 'headline_size', 'caption_size',
                   ...
                   'bottom_logo_size', 'mid_container_margin', 'mid_container_padding',
                   'breaking_box_width', 'breaking_box_height', 'caption_padding', 'caption_margin'):
            try:
                val = int(val)
