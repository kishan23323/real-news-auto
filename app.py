...
function buildAccurateTemplateSVG(style, opts) {
  ...
  return `
    ...
    <rect x="0" y="${midY}" width="${W}" height="${MID_H}" rx="${s.container_radius}" fill="${s.frame_color}"${click('midContainer')}/>

    <g${click('imageArea')}>
      <rect x="30" y="${midY + 40}" width="480" height="${MID_H - 80}" rx="${s.corner_radius}" fill="#FFFFFF"/>
      <rect x="${30 + s.image_border}" y="${midY + 40 + s.image_border}" width="${480 - s.image_border*2}" height="${MID_H - 80 - s.image_border*2}" rx="${Math.max(0, s.corner_radius - s.image_border)}" fill="#555555"/>
      <text x="270" y="${midY + MID_H/2}" font-size="16" fill="#ddd" text-anchor="middle" pointer-events="none">IMAGE/VIDEO</text>
    </g>

    <g${click('breakingBox')}>
      <rect x="550" y="${midY + 40}" width="680" height="${s.headline_size + 20}" rx="${(s.headline_size + 20)/2}" fill="${s.breaking_bg}" stroke="${s.breaking_outline}" stroke-width="3"/>
      <text x="890" y="${midY + 40 + (s.headline_size + 20)/2 + 8}" font-size="${s.headline_size*0.7}" fill="${s.breaking_text_color}" text-anchor="middle" font-weight="bold" pointer-events="none">${s.breaking_text}</text>
    </g>

    <rect x="550" y="${midY + s.headline_size + 70}" width="680" height="${MID_H - s.headline_size - 110}" rx="${s.caption_radius}" fill="${s.banner_color}"${click('captionBox')}/>
    <text x="890" y="${midY + s.headline_size + 70 + (MID_H - s.headline_size - 110)/2 + 6}" font-size="${s.caption_size*0.8}" fill="#000000" text-anchor="middle" pointer-events="none">Caption text here</text>
