        ...
        <div class="approval-panel" id="template-panel">
          ...
          <div class="tpl-style-panel">
            ...
            <div class="tpl-style-grid">
              ...
              <label class="tpl-ctrl">
                Image corner radius <span id="val-corner-radius">0px</span>
...
function showTemplatePanel(templateList, jobId, defaultStyle) {
  ...
  _currentStyle  = Object.assign({
    logo_position: 'top-right', banner_color: '#FFD100', bottom_color: '#121212',
    frame_color: '#C81414', corner_radius: 0, headline_size: 64, caption_size: 30,
  }, defaultStyle || {});
...
function buildTemplateSVG(t, s) {
  ...
  const logoW = 26, logoH = topH * 0.65;
  const logoX = logoLeft ? ox + 6 : ox + w - logoW - 6;
  const isSquareStack = (t.key !== 'landscape_16_9'); // shorts/square use stacked layout

  let mid = '';
  if (!isSquareStack) {
    // landscape: left image, right BREAKING+caption
    const pad = midH * 0.10, contentH = midH - 2 * pad, halfW = (w - 3 * 8) / 2;
    const cy = oy + topH + pad;
    const bH = contentH * 0.18;
    mid += `<rect x="${ox+8}" y="${cy}" width="${halfW}" height="${contentH}" rx="${Math.min(s.corner_radius*scale,halfW/2)}" fill="#555"/>`;
    mid += `<text x="${ox+8+halfW/2}" y="${cy+contentH/2}" font-size="9" fill="#ddd" text-anchor="middle">IMAGE</text>`;
    const rx = ox + 8 + halfW + 8;
    mid += `<rect x="${rx}" y="${cy}" width="${halfW}" height="${bH}" fill="${s.frame_color}"/>`;
    mid += `<text x="${rx+halfW/2}" y="${cy+bH/2+3}" font-size="${Math.max(6,s.headline_size*scale*0.5)}" fill="#fff" text-anchor="middle" font-weight="bold">BREAKING</text>`;
    mid += `<rect x="${rx}" y="${cy+bH+4}" width="${halfW}" height="${contentH-bH-4}" fill="${s.banner_color}"/>`;
    mid += `<text x="${rx+halfW/2}" y="${cy+bH+4+(contentH-bH-4)/2+3}" font-size="${Math.max(6,s.caption_size*scale*0.7)}" fill="#000" text-anchor="middle">Caption text</text>`;
  } else {
    // stacked: BREAKING bar, image, caption — full width
    const pad = midH * 0.06, contentH = midH - 2 * pad, cx0 = ox + 6, cw = w - 12;
    const cy = oy + topH + pad;
    const bH = contentH * 0.09, imgH = contentH * 0.52, gap = 4;
    mid += `<rect x="${cx0}" y="${cy}" width="${cw}" height="${bH}" fill="${s.frame_color}"/>`;
    mid += `<text x="${cx0+cw/2}" y="${cy+bH/2+3}" font-size="${Math.max(6,s.headline_size*scale*0.45)}" fill="#fff" text-anchor="middle" font-weight="bold">BREAKING</text>`;
    const iy = cy + bH + gap;
    mid += `<rect x="${cx0}" y="${iy}" width="${cw}" height="${imgH}" rx="${Math.min(s.corner_radius*scale,imgH/2)}" fill="#555"/>`;
    mid += `<text x="${cx0+cw/2}" y="${iy+imgH/2}" font-size="9" fill="#ddd" text-anchor="middle">IMAGE</text>`;
    const yy = iy + imgH + gap, yh = contentH - bH - imgH - 2*gap;
    mid += `<rect x="${cx0}" y="${yy}" width="${cw}" height="${yh}" fill="${s.banner_color}"/>`;
    mid += `<text x="${cx0+cw/2}" y="${yy+yh/2+3}" font-size="${Math.max(6,s.caption_size*scale*0.6)}" fill="#000" text-anchor="middle">Caption text</text>`;
  }
  ...
  return `<svg viewBox="0 0 ${vw} ${vh}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="${logoX}" y="${oy+(topH-logoH)/2}" width="${logoW}" height="${logoH}" fill="#333" rx="2"/>
    <text x="${logoX+logoW/2}" y="${oy+topH/2+2}" font-size="5" fill="#fff" text-anchor="middle">LOGO</text>
    <text x="${logoLeft ? ox+logoW+10 : ox+6}" y="${oy+topH/2+3}" font-size="6" fill="#000" text-anchor="start">headline text…</text>
    ${mid}
    <rect x="${ox}" y="${oy+h-botH}" width="${w}" height="${botH}" fill="${s.bottom_color}"/>
    <rect x="${ox+4}" y="${oy+h-botH+3}" width="${botH*0.8}" height="${botH-6}" fill="#900" rx="2"/>
...
def approve_templates(job_id):
    ...
    for key, default_val in DEFAULT_STYLE.items():
        ...
        if key == 'logo_position' and val not in ('top-left', 'top-right'):
            continue
        if key in ('corner_radius', 'headline_size', 'caption_size'):
            try:
                val = int(val)
