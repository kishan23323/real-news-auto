...
function editorSaveDefaults() {
  saveEditorLayout();
  // Sync COMPLETE layout to the template style system
  const synthStyle = {
    // Colors & visual
    banner_color: _editorLayout.topBanner.bg,
    bottom_color: _editorLayout.bottomBanner.bg,
    frame_color: _editorLayout.midContainer.bg,
    body_bg: _editorLayout.canvas.bg,
    logo_position: _editorLayout.logo.pos === 'top-left' ? 'top-left' : 'top-right',
    logo_shape: _editorLayout.logo.shape,
    container_radius: _editorLayout.midContainer.radius,
    corner_radius: _editorLayout.imageArea.radius,
    headline_size: _editorLayout.breakingBox.textSize,
    caption_size: _editorLayout.captionBox.textSize,
    ...
    // LOGO
    logo_size: _editorLayout.logo.size,
    logo_bg: _editorLayout.logo.bg,
    logo_border: _editorLayout.logo.border,
    
    // MIDDLE CONTAINER
    mid_border_width: _editorLayout.midContainer.border || 0,
    
    // IMAGE AREA
    image_border: _editorLayout.imageArea.border,
    image_bg: _editorLayout.imageArea.bg,
    image_gradient_alpha: _editorLayout.imageArea.gradientAlpha,
    
    // BREAKING NEWS
    breaking_text: _editorLayout.breakingBox.text,
    breaking_text_color: _editorLayout.breakingBox.textColor,
    breaking_bg: _editorLayout.breakingBox.bg,
    breaking_outline: _editorLayout.breakingBox.outline,
    
    // CAPTION
    caption_radius: _editorLayout.captionBox.radius,
    
    // BOTTOM BANNER
    bottom_height: _editorLayout.bottomBanner.h,
    bottom_padding: _editorLayout.bottomBanner.mt,
    bottom_margin_lr: _editorLayout.bottomBanner.ml,
    bottom_text: _editorLayout.bottomBanner.text,
    bottom_text_size: _editorLayout.bottomBanner.textSize,
    bottom_text_color: _editorLayout.bottomBanner.textColor,
    bottom_logo_size: _editorLayout.bottomBanner.logoSize,
  };
...
function renderEditorPreview() {
  ...
  const L = _editorLayout;
  const W = 1280, H = 720;
  const MID_H = H - L.topBanner.h - L.bottomBanner.h;
  const midY = L.topBanner.h;

  let markup = `
    <!-- Canvas / Body Background (click empty margin areas to select) -->
    <rect id="e-canvas" x="0" y="0" width="${W}" height="${H}" fill="${L.canvas.bg}" onclick="editorSelect('canvas')" style="cursor:pointer;"/>

    <!-- Top Banner -->
    <rect id="e-top" x="0" y="0" width="${W}" height="${L.topBanner.h}" fill="${L.topBanner.bg}" onclick="editorSelect('topBanner')"/>
    <text id="e-top-text" x="20" y="${L.topBanner.h/2+6}" font-size="${L.topBanner.textSize}" fill="${L.topBanner.textColor}" onclick="editorSelect('topBanner')" style="cursor:pointer;font-weight:bold;">आज की 50 बड़ी खबरें</text>
    
    <!-- Logo -->
    <g id="e-logo" onclick="editorSelect('logo')" style="cursor:pointer;">
      ${L.logo.shape === 'circle' 
        ? `<circle cx="${L.logo.pos === 'top-right' ? W - L.logo.size/2 - 20 : L.logo.size/2 + 20}" cy="${L.topBanner.h/2}" r="${L.logo.size/2}" fill="${L.logo.bg}" stroke="${L.logo.border}" stroke-width="2"/>`
        : `<rect x="${L.logo.pos === 'top-right' ? W - L.logo.size - 20 : 20}" y="${L.topBanner.h/2 - L.logo.size/2}" width="${L.logo.size}" height="${L.logo.size}" rx="4" fill="${L.logo.bg}" stroke="${L.logo.border}" stroke-width="2"/>`
      }
      <text x="${L.logo.pos === 'top-right' ? W - L.logo.size/2 - 20 : L.logo.size/2 + 20}" y="${L.topBanner.h/2+4}" font-size="12" fill="#333" text-anchor="middle" onclick="editorSelect('logo')" style="pointer-events:none;">LOGO</text>
    </g>
    
    <!-- Middle Container (red bg) -->
    <rect id="e-mid" x="0" y="${midY}" width="${W}" height="${MID_H}" rx="${L.midContainer.radius}" fill="${L.midContainer.bg}" onclick="editorSelect('midContainer')"/>
    
    <!-- Image Area (white bordered, rounded) -->
    <g id="e-image" onclick="editorSelect('imageArea')">
      <rect x="30" y="${midY + 40}" width="480" height="${MID_H - 80}" rx="${L.imageArea.radius}" fill="#FFFFFF" stroke="none"/>
      <rect x="${30 + L.imageArea.border}" y="${midY + 40 + L.imageArea.border}" width="${480 - L.imageArea.border*2}" height="${MID_H - 80 - L.imageArea.border*2}" rx="${Math.max(0, L.imageArea.radius - L.imageArea.border)}" fill="#555555"/>
      <text x="270" y="${midY + MID_H/2}" font-size="16" fill="#ddd" text-anchor="middle" pointer-events="none">IMAGE/VIDEO</text>
    </g>
    
    <!-- BREAKING NEWS Pill (with outline so it's visible even if bg matches surroundings) -->
    <g id="e-breaking" onclick="editorSelect('breakingBox')" style="cursor:pointer;">
      <rect x="550" y="${midY + 40}" width="680" height="${L.breakingBox.textSize + 20}" rx="${(L.breakingBox.textSize + 20)/2}" fill="${L.breakingBox.bg}" stroke="${L.breakingBox.outline}" stroke-width="3"/>
      <text x="890" y="${midY + 40 + (L.breakingBox.textSize + 20)/2 + 8}" font-size="${L.breakingBox.textSize*0.7}" fill="${L.breakingBox.textColor}" text-anchor="middle" font-weight="bold" pointer-events="none">BREAKING NEWS</text>
    </g>
    
    <!-- Caption Box (yellow) -->
    <rect id="e-caption" x="550" y="${midY + L.breakingBox.textSize + 70}" width="680" height="${MID_H - L.breakingBox.textSize - 110}" rx="${L.captionBox.radius}" fill="${L.captionBox.bg}" onclick="editorSelect('captionBox')" style="cursor:pointer;"/>
    <text x="890" y="${midY + L.breakingBox.textSize + 70 + (MID_H - L.breakingBox.textSize - 110)/2 + 6}" font-size="${L.captionBox.textSize*0.8}" fill="${L.captionBox.textColor}" text-anchor="middle" pointer-events="none">Caption text here</text>
    
    <!-- Bottom Banner -->
    <rect id="e-bottom" x="0" y="${H - L.bottomBanner.h}" width="${W}" height="${L.bottomBanner.h}" fill="${L.bottomBanner.bg}" onclick="editorSelect('bottomBanner')" style="cursor:pointer;"/>
    <circle cx="${14 + L.bottomBanner.logoSize/2}" cy="${H - L.bottomBanner.h/2}" r="${L.bottomBanner.logoSize/2}" fill="#FFFFFF" stroke="#333333" stroke-width="2" onclick="editorSelect('bottomBanner')" style="cursor:pointer;"/>
    <text x="${14 + L.bottomBanner.logoSize + 20}" y="${H - L.bottomBanner.h/2 + 8}" font-size="${L.bottomBanner.textSize*0.9}" fill="${L.bottomBanner.textColor}" onclick="editorSelect('bottomBanner')" style="cursor:pointer;pointer-events:auto;">Share text here</text>
  `;
