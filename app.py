...
const LAYOUT_DEFAULTS = {
  ...
  breakingBox: { bg: '#C81414', textSize: 64, textColor: '#FFFFFF', text: 'BREAKING NEWS' },
  captionBox: { bg: '#FFD100', textSize: 30, textColor: '#000000', radius: 12 },
  bottomBanner: { h: 72, bg: '#121212', text: 'अपने दोस्तों के साथ फेसबुक और व्हाट्सएप पर शेयर जरूर करें', textSize: 28, textColor: '#FFFFFF' },
};
...
function editorSaveDefaults() {
  saveEditorLayout();
  // Also sync to the template style system
  const synthStyle = {
  ...
  alert('✓ Layout saved as your default');
}
...
function renderEditorPreview() {
  ...
  let markup = `
    ...
    <g id="e-breaking" onclick="editorSelect('breakingBox')" style="cursor:pointer;">
      <rect x="550" y="${midY + 40}" width="680" height="${L.breakingBox.textSize + 20}" rx="${(L.breakingBox.textSize + 20)/2}" fill="${L.breakingBox.bg}"/>
    ...
    <text x="20" y="${H - L.bottomBanner.h/2 + 8}" font-size="${L.bottomBanner.textSize*0.9}" fill="${L.bottomBanner.textColor}" onclick="editorSelect('bottomBanner')" style="cursor:pointer;pointer-events:auto;">Share text here</text>
  `;
...
function renderEditorProps() {
  ...
  for (const [prop, val] of Object.entries(elem)) {
    ...
    if (prop.includes('Color') || prop === 'bg') {
