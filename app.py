  ...
  <div class="main">
    ...
    <div class="panel active" id="panel-generate">
      <div class="gen-card">
        ...
        <!-- Template picker — shows after image approval -->
        <div class="approval-panel" id="template-panel">
          <h3>🎬 Choose Template(s)</h3>
          <p>Pick which video format(s) to build. Only the templates you select will be rendered.</p>
          <div class="sentence-list" id="template-list"></div>
          <div class="approval-actions">
            <button class="btn-approve" onclick="approveTemplates()">
              ✅ Approve Template(s) &amp; Build Video
            </button>
          </div>
        </div>
...
function showTemplatePanel(templateList, jobId) {
  ...
  (templateList || []).forEach((t, i) => {
    ...
    row.style.cssText = 'display:flex;align-items:center;gap:10px;padding:10px 4px;border-bottom:1px solid #2a2a2a;cursor:pointer;';
    row.innerHTML = `
      <input type="checkbox" class="template-checkbox" value="${t.key}" ${i === 0 ? 'checked' : ''} style="width:16px;height:16px;">
      <span style="flex:1;">
        <strong>${t.label}</strong>
        <span style="color:var(--sub);font-size:12px;"> — ${t.size[0]}×${t.size[1]}</span>
      </span>`;
...
function pollJob(jobId) {
  ...
  es.onmessage = e => {
    ...
    if (d.status === 'waiting_approval') {
      ...
      showTemplatePanel(d.templates || [], d.job_id || jobId);
      document.querySelector('#template-panel .btn-approve').addEventListener('click', () => {
        setTimeout(() => pollJob(jobId), 600);
      }, {once: true});
...
def _run_pipeline(job_id, url, topic, minutes):
    ...
    try:
        ...
        emit(3, 67, 'Choose which template(s) to build', status='waiting_template_approval',
             templates=list_templates(), job_id=job_id)
