...
16:9 (landscape_16_9)              9:16 / 1:1 (shorts_9_16, square_1_1)
┌───────────────────────────┐      ┌───────────────────┐
│ YELLOW BANNER    [logo]   │      │ YELLOW BANNER [lg] │
├───────────────────────────┤      ├───────────────────┤
│┆  ┌──────────┐┌─────────┐ │      │┆ BREAKING NEWS bar │
│┆  │  image   ││BREAKING │ │      │┆ ┌───────────────┐ │
│┆  │  (Ken    ││ NEWS    │ │      │┆ │ image (Ken     │ │
...
# Template registry — add new formats here without touching layout code.
TEMPLATES = {
    "landscape_16_9": {"size": (1280, 720),  "layout": "split",   "label": "YouTube / TV (16:9)"},
    "shorts_9_16":    {"size": (720, 1280),  "layout": "stacked", "label": "Shorts / Reels / Status (9:16)"},
    "square_1_1":     {"size": (1080, 1080), "layout": "stacked", "label": "Feed post (1:1)"},
}
...
def _build_banners(W, H, TOP_H, BOTTOM_H, total_duration, font):
    """Top yellow banner + bottom dark banner — shared across all templates."""
    layers = []

    # TOP: yellow banner + logo
    layers.append(
    ...
    # BOTTOM: dark banner + badge + share text
    layers.append(
    ...
    badge_clip = _load_badge_clip(badge_target_w, badge_h, total_duration)
...
def _ken_burns_clip(img, box_w, box_h, clip_dur, zoom_in, start_time):
...
def _build_split_body(W, MID_H, frame_y, image_paths, captions, durations,
                       total_duration, font, impact_font):
    """16:9 layout: left image panel + right BREAKING NEWS/caption panel."""
    RED_BORDER  = 8
    ...
    for i, (img, dur) in enumerate(zip(image_paths, durations)):
        clip_dur = dur + 0.4
        image_layers.append(_ken_burns_clip(img, HALF_W, CONTENT_H, clip_dur, i % 2 == 0, t))
        t += dur
...
def _build_stacked_body(W, MID_H, frame_y, image_paths, captions, durations,
                         total_duration, font, impact_font):
    """9:16 / 1:1 layout: BREAKING NEWS bar, then image, then caption — all full width, stacked."""
    RED_BORDER  = 8
    ...
    layers.append(
        ColorClip(size=(CONTENT_W, breaking_h), color=RED).with_duration(total_duration)
        .with_position((x0, content_y))
    )
    bn_txt, _ = _autofit_textclip(
        "BREAKING NEWS", CONTENT_W - 30, impact_font, start_size=56, color="white",
    ...
    for i, (img, dur) in enumerate(zip(image_paths, durations)):
        clip_dur = dur + 0.4
        image_layers.append(_ken_burns_clip(img, CONTENT_W, image_h, clip_dur, i % 2 == 0, t))
        t += dur
    ...
    layers.append(
        ColorClip(size=(CONTENT_W, yellow_h), color=YELLOW).with_duration(total_duration)
        .with_position((x0, yellow_y))
    )

    t = 0.0
    for caption, dur in zip(captions, durations):
        clip_dur = dur + 0.3
        cap_clip = _autofit_wrapped(
            _wrap_caption(caption), CONTENT_W - 40, yellow_h - 20, font, 34, "black",
...
def build_video(image_paths, captions, durations, audio_path, title,
                 out_path="output.mp4", template="landscape_16_9", lang="hi",
                 tmp_dir="/tmp"):
    """
    template: one of TEMPLATES.keys() — "landscape_16_9", "shorts_9_16", "square_1_1"
    """
    if template not in TEMPLATES:
        raise ValueError(f"Unknown template '{template}'. Choose from: {list(TEMPLATES.keys())}")
    W, H = TEMPLATES[template]["size"]
    ...
    layers = _build_banners(W, H, TOP_H, BOTTOM_H, total_duration, font)

    if layout == "split":
        layers += _build_split_body(
            W, MID_H, TOP_H, image_paths, captions, durations,
            total_duration, font, impact_font,
        )
    else:
        layers += _build_stacked_body(
            W, MID_H, TOP_H, image_paths, captions, durations,
            total_duration, font, impact_font,
        )
