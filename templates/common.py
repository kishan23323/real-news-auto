...
DEFAULT_STYLE = {
    ...
    "frame_color":      "#C81414",   # red border/frame around content + BREAKING NEWS bar
    "logo_position":    "top-right", # "top-right" | "top-left"
    "corner_radius":    0,           # rounded corners on the image panel(s), in px (canvas-relative)
    "headline_size":    64,          # "BREAKING NEWS" bar starting font size
...
def build_banners(W, H, TOP_H, BOTTOM_H, total_duration, font, style=None):
    """Top banner + bottom banner — shared across all templates. Colors and
    logo position come from `style` (see DEFAULT_STYLE)."""
    ...
    logo_x = 16 if logo_left else W - logo_w - 16
    if logo_clip:
        layers.append(logo_clip.with_position((logo_x, (TOP_H - logo_h) // 2)))
    text_x = logo_w + 34 if logo_left else 20
    text_max_w = W - logo_w - 50
    top_headline = autofit_wrapped(
        TOP_TEXT_HI, text_max_w, TOP_H - 10, font, 32, "black",
        min_size=16, align="left",
    )
    layers.append(top_headline.with_position((text_x, (TOP_H - top_headline.size[1]) // 2)))
