...
def wrap_text_no_split(text, font_path, font_size, max_width):
    """Wrap text into lines, breaking ONLY at spaces (whole word
    boundaries) — never splits a word/syllable mid-character. Moviepy's
    built-in method="caption" auto-wrap measures by naive character
    count and can break Devanagari words apart (e.g. "कहना" -> "कह" /
    "ना"); this measures actual rendered pixel width per word instead."""
    ...
    for word in words:
        ...
        if not current or text_w(candidate) <= max_width:
            current = candidate
        else:
...
def autofit_wrapped(text, max_width, max_height, font, start_size, color,
    ...
    """Like autofit_textclip but for wrapped multi-line text — wraps at
    word boundaries only (never mid-word) via wrap_text_no_split, then
    renders with method='label' so moviepy doesn't re-wrap on top of it."""
    size = start_size
    while size > min_size:
        wrapped = wrap_text_no_split(text, font, size, max_width)
        kw = dict(text=wrapped, font_size=size, color=color, method="label", text_align=align)
        if font:
            kw["font"] = font
        clip = TextClip(**kw).with_duration(0.1)
        if clip.size[1] <= max_height:
            return clip
        size -= step
    wrapped = wrap_text_no_split(text, font, min_size, max_width)
    kw = dict(text=wrapped, font_size=min_size, color=color, method="label", text_align=align)
    if font:
        kw["font"] = font
...
DEFAULT_STYLE = {
    ...
    # MIDDLE CONTAINER
    "mid_border_width": 0,
    "mid_container_margin_top":    0,    # gap between container and top banner
    "mid_container_margin_right":  22,
    "mid_container_margin_bottom": 0,    # gap between container and bottom banner
    "mid_container_margin_left":   22,
    "mid_container_padding_top":    0,   # 0 = auto (keeps content clear of rounded corners)
    "mid_container_padding_right":  0,
    "mid_container_padding_bottom": 0,
    "mid_container_padding_left":   0,
    ...
    "breaking_box_width":  0,      # 0 = auto-size to text
    "breaking_box_height": 0,      # 0 = auto-size to text
    "breaking_font":     "impact", # "impact" | "devanagari" | "system"
    "breaking_text_align": "center",  # "left" | "center" | "right"
    
    # CAPTION
    "caption_radius":   12,
    "caption_padding_top":    20,   # inset between caption box edge and its text
    "caption_padding_right":  20,
    "caption_padding_bottom": 20,
    "caption_padding_left":   20,
    "caption_margin_top":     18,   # gap between the BREAKING NEWS pill and the caption box
...
FONT_CHOICES = {
    "impact":     lambda: IMPACT_FONT if os.path.exists(IMPACT_FONT) else None,
    "devanagari": lambda: BUNDLED_FONT if os.path.exists(BUNDLED_FONT) else None,
    "system":     lambda: None,  # PIL default sans-serif
}


def resolve_font_choice(key, fallback):
    """Map a UI font-choice key ('impact' | 'devanagari' | 'system') to an
    actual font file path, falling back to `fallback` if unset/unknown/missing."""
    if not key or key not in FONT_CHOICES:
        return fallback
    path = FONT_CHOICES[key]()
    return path if (path is None or os.path.exists(path)) else fallback


def pill_clip(text, font, font_size, bg_hex, fg_hex, pad_x, pad_y, duration, min_size=18,
              outline_hex="#FFFFFF", outline_width=3, fixed_width=None, fixed_height=None,
              align="center"):
    ...
    happens to match the color behind it. If fixed_width/fixed_height are
    given, the box uses that exact size (text still autofits inside it)
    instead of auto-sizing to the text."""
    avail_w = (fixed_width - pad_x * 2) if fixed_width else 10_000
    txt, used_size = autofit_textclip(text, avail_w, font, font_size, color=fg_hex,
                                       min_size=min_size, method="label")
    txt = txt.with_duration(duration)
    tw, th = txt.size
    box_w = fixed_width if fixed_width else tw + pad_x * 2
    box_h = fixed_height if fixed_height else th + pad_y * 2
    radius = box_h // 2
    ...
    if align == "left":
        ...
        text_x = box_w - tw - pad_x
    else:
        text_x = (box_w - tw) // 2

    return CompositeVideoClip(
        [bg, txt.with_position((text_x, (box_h - th) // 2))],
        size=(box_w, box_h),
...
def build_banners(W, H, TOP_H, BOTTOM_H, total_duration, font, style=None):
    ...
    # Bottom-left graphic: prefer a custom-uploaded logo if the user set
    # one; otherwise use the bundled breaking_news_badge.png (the actual
    # reference-design asset) if it exists; otherwise fall back to the
    # same circular/square logo used up top.
    custom_logo = style.get("logo_path") or None
    if custom_logo and os.path.exists(custom_logo):
        if circular:
            ...
            bottom_logo_clip = load_logo_clip(min(bottom_logo_size, BOTTOM_H - 8), total_duration,
                                               logo_path=custom_logo)
    elif os.path.exists(BADGE_PATH):
        bottom_logo_clip = load_badge_clip(min(240, round(W * 0.19)), min(bottom_logo_size, BOTTOM_H - 8), total_duration)
    elif circular:
        bottom_logo_clip = circular_logo_clip(min(bottom_logo_size, BOTTOM_H - 8), total_duration,
                                               ring_color=style.get("logo_border", "#333333"))
    else:
