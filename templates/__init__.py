...
def render(template_key, image_paths, captions, durations, audio_path,
    ...
    resolved_style = common.resolve_style(style)
    TOP_H    = int(resolved_style.get("top_height", round(H * 0.10)))
    BOTTOM_H = int(resolved_style.get("bottom_height", round(H * 0.10)))
    # Safety clamp: banners can never together exceed the frame, and each
    # needs at least a sane minimum to hold text/logo.
    TOP_H = max(60, min(TOP_H, int(H * 0.35)))
    BOTTOM_H = max(50, min(BOTTOM_H, int(H * 0.35)))
    if TOP_H + BOTTOM_H > H - 100:
        scale = (H - 100) / (TOP_H + BOTTOM_H)
        TOP_H = int(TOP_H * scale)
        BOTTOM_H = int(BOTTOM_H * scale)
