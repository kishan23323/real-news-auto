...
def build_body(W, MID_H, frame_y, image_paths, captions, durations,
    ...
    layers.append(
        ColorClip(size=(CONTENT_W, breaking_h), color=common.RED).with_duration(total_duration)
    ...
    for i, (img, dur) in enumerate(zip(image_paths, durations)):
        clip_dur = dur + 0.4
        image_layers.append(common.ken_burns_clip(img, CONTENT_W, image_h, clip_dur, i % 2 == 0, t))
        t += dur
    ...
    layers.append(
        ColorClip(size=(CONTENT_W, yellow_h), color=common.YELLOW).with_duration(total_duration)
    ...
    for caption, dur in zip(captions, durations):
        ...
        cap_clip = common.autofit_wrapped(
            common.wrap_caption(caption), CONTENT_W - 40, yellow_h - 20, font, 34, "black",
