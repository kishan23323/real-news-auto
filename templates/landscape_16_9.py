...
def build_body(W, MID_H, frame_y, image_paths, captions, durations,
    ...
    for i, (img, dur) in enumerate(zip(image_paths, durations)):
        clip_dur = dur + 0.4
        image_layers.append(common.ken_burns_clip(img, HALF_W, CONTENT_H, clip_dur, i % 2 == 0, t, corner_radius))
        t += dur
    ...
    layers.append(
        ColorClip(size=(HALF_W, breaking_h), color=frame_rgb).with_duration(total_duration)
        .with_position((right_x, content_y))
    )
    bn_txt, _ = common.autofit_textclip(
        "BREAKING NEWS", HALF_W - 30, impact_font, start_size=headline_size, color="white",
    ...
    layers.append(
        ColorClip(size=(HALF_W, yellow_h), color=common._hex_to_rgb(style["banner_color"], common.YELLOW))
        .with_duration(total_duration).with_position((right_x, yellow_y))
    )
    ...
    for caption, dur in zip(captions, durations):
        ...
        cap_clip = common.wrapped_text_clip(
            common.wrap_caption(caption), HALF_W - 40, font, caption_size, "black", clip_dur, align="center",
        )
